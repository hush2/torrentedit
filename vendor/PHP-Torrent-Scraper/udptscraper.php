    ...
    public function scrape($url, $infohash)
    {
        ...
        if (strlen($ret) < 1) { //throw new ScraperException('No scrape response.',0,true);
            ...
            foreach ($infohash as $hash) {
                ...
                $left = $downloaded;
                $uploaded = $downloaded;
                $packet = $current_connid . pack("N", 1) . pack("N", $transaction_id) . pack('H*',$hash). pack('H*','ee3eb1acec1dc7adc73eda16d05a495bea1ddab1').$downloaded.$left.$uploaded.pack("N", 2).pack("N", ip2long($_SERVER['SERVER_ADDR'])).pack("N", 69).pack("N", 500).pack("N", rand(0,65535));
                ...
                if(strlen($ret) < 1){ throw new ScraperException('No announce response.',0,true); }
                if(strlen($ret) < $readlength){ throw new ScraperException('Too short announce response.'); }
                $retd = unpack("Naction/Ntransid",$ret);
                // Todo check for error string if response = 3
                if($retd['action'] != 2 || $retd['transid'] != $transaction_id){
                    throw new ScraperException('Invalid announce response.');
                }
